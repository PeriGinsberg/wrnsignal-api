...
foreach ($row in $rows) {
   $targetFamilies = @()
$targetText = (($row.target_roles + " " + $row.adjacent_roles) | Out-String).ToLower()

if ($targetText -match "marketing|brand|product marketing|digital marketing|communications|content|social media|creative") { $targetFamilies += "Marketing" }
if ($targetText -match "finance|wealth|investment|asset management|capital markets|investor relations|financial analyst|commercial real estate") { $targetFamilies += "Finance" }
if ($targetText -match "accounting|financial reporting|audit|assurance|tax") { $targetFamilies += "Accounting" }
if ($targetText -match "analytics|data analyst|business intelligence|bi\b|sql|tableau|power bi") { $targetFamilies += "Analytics" }
if ($targetText -match "consulting|strategy consulting|management consulting") { $targetFamilies += "Consulting" }
if ($targetText -match "legal|legal intake|legal services|legal assistant|paralegal|privacy|data protection|regulatory") { $targetFamilies += "Other" }
if ($targetText -match "government|government affairs|public policy|legislative") { $targetFamilies += "Government" }
if ($targetText -match "premed|clinical|healthcare|medical|life sciences|biology") { $targetFamilies += "PreMed" }
if ($targetText -match "(?<!non-)sales|business development|account executive|\bbdr\b|\bsdr\b") { $targetFamilies += "Sales" }
if ($targetFamilies.Count -eq 0) {
    if ($targetText -match "design|designer|ux|ui|product design|graphic design|visual design|brand design|environmental graphics") {
        $targetFamilies += "Other"
    }
    elseif ($targetText -match "legal|privacy|data protection|legal intake|legal services|legal assistant|paralegal") {
        $targetFamilies += "Other"
    }
    elseif ($targetText -match "operations|process|process improvement|transformation|implementation|business analyst|program management|project management") {
        $targetFamilies += "Other"
    }
    elseif ($targetText -match "advisor|client associate|wealth management") {
        $targetFamilies += "Finance"
    }
    else {
        $targetFamilies += "Other"
    }
...
"@
    ...
    $caseObject = [ordered]@{
        ...
        profileOverrides = [ordered]@{
            ...
            statedInterests = [ordered]@{
                targetRoles = @((($row.target_roles -split ";|,") | ForEach-Object { $_.Trim() } | Where-Object { $_ }))
                adjacentRoles = @((($row.adjacent_roles -split ";|,") | ForEach-Object { $_.Trim() } | Where-Object { $_ }))
                targetIndustries = @((($row.target_industries -split ";|,") | ForEach-Object { $_.Trim() } | Where-Object { $_ }))
            }
	    tools = @($tools)
            gradYear = $gradYear
