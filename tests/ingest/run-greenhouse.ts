/**
 * Phase 1 step 3 run: one pair, five Greenhouse boards.
 *
 * Greenhouse has NO server-side keyword filter, so filtering is local and the
 * control asserts our own predicate narrows rather than that the source did.
 * See lib/ingest/greenhouse.ts.
 *
...
// Five real Greenhouse boards taken from lane_results.
const DEFAULT_ORGS = ["capco", "point72", "janestreet", "mediabrands", "scaleai"]
...
;(async () => {
  ...
  const { data, error } = await sb
    ...
    .eq("source", greenhouseAdapter.source)
  ...
  for (const r of data || []) {
    ...
    if (c) {
      console.log("  control        filtering=" + c.filtering + "  term=" + c.term)
      if (c.filtering === "local") {
        console.log("                 unfiltered=" + c.unfiltered + "  filtered=" + c.filtered + "  nonsense=" + c.nonsense +
          "  narrows=" + c.narrows + "  rejects=" + c.rejects + (c.empty_board ? "  EMPTY BOARD" : ""))
      } else {
        console.log("                 totalFound=" + c.totalFound + "  returned=" + c.returned)
      }
    }
  }
