...
/**
 * Per-attempt AI cost in cents (PLACEHOLDER for v0.1).
 *
 * Used by /draft endpoint to increment phase2_runs.ai_cost_cents on
 * every generation attempt (including retries). Stage 2c will swap this
 * fixed-cost-per-attempt model for usage-derived token cost
 * (input_tokens * input_rate + output_tokens * output_rate when real
 * Claude Haiku integration lands).
 *
 * Until then, 1 cent/attempt provides a coarse but functional
 * cap-enforcement mechanism. With MAX_COST_CENTS=50 this gives users
 * ~50 attempts per phase2_run before the cap fires — well above typical
 * workflow usage.
 */
