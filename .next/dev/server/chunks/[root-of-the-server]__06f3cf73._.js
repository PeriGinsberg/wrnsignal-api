...
;
// -----------------------------
// CORS (inline to avoid import mismatches)
// -----------------------------
const CORS_HEADERS = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "POST,OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, x-jobfit-test-key"
};
function withCorsJson(_req, data, status = 200) {
...
async function POST(req) {
    ...
    try {
        // Parse JSON first so bypass can work without auth.
        ...
        if (bypass) {
            ...
            if (!profileText) {
                return withCorsJson(req, {
                    error: "Missing profileText (or profile)"
                }, 400);
            }
            console.log("[jobfit/route] about to evaluate", {
                mode: body?.mode,
                hasProfileText: !!body?.profileText,
                hasProfile: !!body?.profile,
                jobLen: (body?.job || "").length
            });
            ...
            return withCorsJson(req, raw, 200);
        }
        console.log("[jobfit/route] evaluated result", {
            decision: result?.decision,
            score: result?.score,
            riskFlagsCount: (result?.risk_flags || []).length,
            whyCodesCount: (result?.why_codes || []).length,
            gateType: result?.gate_triggered?.type
        });
        // -----------------------------
        // NORMAL path (requires bearer)
        // -----------------------------
        const authed = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$api$2f$_lib$2f$authProfile$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAuthedProfileText"])(req) // expects Authorization: Bearer ...
        ;
        const profileText = String(authed?.profileText || "").trim();
        const userId = authed?.userId;
        if (!profileText) {
            return withCorsJson(req, {
                error: "Unauthorized or missing profile text"
            }, 401);
        }
        const raw = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$api$2f$_lib$2f$jobfitEvaluator$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["runJobFit"])({
            profileText,
        ...
        // Keep errors JSON so curl testing is stable.
...
//# sourceMappingURL=%5Broot-of-the-server%5D__06f3cf73._.js.map