...
function computeBaseScore(job: StructuredJobSignals, profile: StructuredProfileSignals): number {
  ...
  // Structure signals help differentiate "real" postings from vague ones
  const structureSignals =
    (job.yearsRequired !== null ? 1 : 0) +
    (job.location?.mode && job.location.mode !== "unclear" ? 1 : 0) +
    ((job.requiredTools?.length || 0) + (job.preferredTools?.length || 0) > 0 ? 1 : 0)

  if (familyMatch) {
    base += Math.min(4, structureSignals * 2)
  }
  ...
  if (hasExplicitTools) {
    ...
    const bump = Math.min(6, overlap.length * 2)
    base += bump
  }
...
// Role clarity WHY
const roleClarityCount =
  (job.yearsRequired !== null ? 1 : 0) +
  ((job.requiredTools?.length || 0) > 0 ? 1 : 0) +
  ((job.preferredTools?.length || 0) > 0 ? 1 : 0) +
  (job.location?.mode && job.location.mode !== "unclear" ? 1 : 0)

if (familyMatch && roleClarityCount >= 2) {
  whyCodes.push({
    code: "WHY_ROLE_CLARITY",
    job_fact: "The posting gives concrete detail on requirements, tools, or work setup.",
    profile_fact: "Clearer roles are easier to evaluate and target effectively.",
    note: "This role is defined clearly enough that you can position yourself against it with confidence.",
    weight: 3,
  })
}

// Functional alignment WHY
const functionalEvidenceCount =
  (job.reportingSignals?.strong ? 1 : 0) +
  (job.analytics?.isLight ? 1 : 0) +
  ((job.internship?.departments?.length || 0) > 0 ? 1 : 0)

if (familyMatch && functionalEvidenceCount >= 2) {
  whyCodes.push({
    code: "WHY_FUNCTIONAL_ALIGNMENT",
    job_fact: "The posting includes multiple concrete signals about the actual work.",
    profile_fact: `This role aligns with your targeted ${job.jobFamily} direction.`,
    note: "The day-to-day responsibilities look directionally aligned with the kind of work you want.",
    weight: 4,
  })
}
// Location WHY (only when profile is constrained and job city is known and matches)
{
  ...
  if (
    ...
    whyCodes.push({
      ...
      profile_fact: `Your preferred locations include ${allowedCities.join(", ")}.`,
      note: "This role lines up with where you are actually willing to work.",
      weight: 4,
    })
  }
}
...
/* ---------------- ADDITIONAL WHY SIGNALS ---------------- */

// Adjacent family (role is not primary target but still reasonable)
if (!familyMatch && profile.targetFamilies.length > 0 && job.jobFamily !== "Other") {
  whyCodes.push({
    code: "WHY_ADJACENT_FAMILY_MATCH",
    job_fact: `Role classified as ${job.jobFamily}.`,
    profile_fact: `Your targets include ${profile.targetFamilies.join(", ")}.`,
    note: "This role sits adjacent to the type of work you are targeting.",
    weight: 6,
  })
}

// Role structure clarity (well defined posting)
if (job.yearsRequired !== null || (job.requiredTools?.length || 0) > 0) {
  whyCodes.push({
    code: "WHY_ROLE_STRUCTURE_CLEAR",
    job_fact: "Posting includes clear experience or tool expectations.",
    profile_fact: "Structured roles are easier to evaluate and compete for.",
    note: "The role description provides clear expectations.",
    weight: 2,
  })
}

// Early career friendly signals
if (
  ...
  whyCodes.push({
    code: "WHY_EARLY_CAREER_FIT",
    job_fact: `Posting asks for around ${job.yearsRequired} years of experience.`,
    profile_fact: `Your profile shows approximately ${profile.yearsExperienceApprox} years.`,
    note: "Experience expectations align with early-career candidates.",
    weight: 4,
  })
}
...
// Experience gap but not hard failure
if (job.yearsRequired !== null && profile.yearsExperienceApprox !== null) {
...
// Weak role clarity - only fire when the posting is truly thin
if (
  job.yearsRequired === null &&
  ...
  !job.internship?.isInternship &&
  !job.internship?.isSummer &&
  !job.internship?.isInPersonExplicit &&
  !job.internship?.departments?.length
) {
  riskOnlyCodes.push({
