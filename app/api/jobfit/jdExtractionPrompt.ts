...
- Map each capability to the SINGLE best key from the vocabulary the user provides. The vocabulary is finite and will not cover every role. When a genuine, real requirement does not fit any provided key, use canonical_key "other" with a clear label and the verbatim text — DO NOT force a bad fit just to use a listed key. Mis-keying a requirement to a WRONG real key is worse than using "other": a wrong real key creates a phantom "missing proof" gap that the candidate is then penalized for, whereas "other" does not. If the single best key is only a loose or approximate fit, prefer "other" with a clear label. An honest "other" is more useful than a wrong bucket.
...
- credentialDetail MUST name a specific credential that appears as a literal span in the job description (e.g. "Series 7", "CPA", "RN", "bar admission", "CDL"). If the posting does not name a specific credential, credentialDetail MUST be null — even when credentialRequired is true (a posting can require "a license" or "certification" without naming which one). NEVER supply a credential label from world knowledge of what the role typically needs; if it is not written in the JD text, it does not exist for this field.
