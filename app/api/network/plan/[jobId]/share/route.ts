...
export async function POST(req: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    // shared_at and client_email_sent_at are reported separately because they
    // mean different things: the first is "the client can see it", the second
    // is "the client has been told". A share can succeed at the first and fail
    // at the second, and the screen has to be able to say so.
    return withCorsJson(req, {
      ok: true,
      shared_at: result.job.shared_at,
      drive_file_url: result.job.drive_file_url,
      client_email_sent_at: result.job.client_email_sent_at ?? null,
      client_email_sent_count: result.job.client_email_sent_count ?? 0,
      client_email_to: result.job.client_email_to ?? null,
      client_email_error: result.job.client_email_error ?? null,
