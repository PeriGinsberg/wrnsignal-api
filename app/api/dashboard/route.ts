...
<div class="metrics">
  <div class="metric">
    ...
    <div class="metric-sub">Unique sessions</div>
  </div>
  <div class="metric">
...
function renderMetrics(rows) {
  ...
  document.getElementById('m-visitors').textContent = uniqueSessions || '0'
  ...
  const intakeRate = uniqueSessions > 0 ? Math.round(intakeCompleted/uniqueSessions*100) : 0
  const runRate = intakeCompleted > 0 ? Math.round(runs/intakeCompleted*100) : 0
  document.getElementById('m-intake-rate').textContent = uniqueSessions > 0 ? \`${intakeRate}% of visitors\` : 'Trial signups'
