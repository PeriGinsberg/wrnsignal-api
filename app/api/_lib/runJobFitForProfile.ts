...
import { runJobFit, detectorFlagsForPath } from "./jobfitEvaluator"
...
export async function runJobFitForProfile(params: {
  ...
  const raw = (await runJobFit({
    ...
    // Detectors for the PAID path (paid authed scan + coach recommend). Dev:
    // JOBFIT_DETECTORS=on. Prod: flipped independently via JOBFIT_DETECTORS_PAID=on
    // (free unaffected). Otherwise {} → byte-identical to today. See detectorFlagsForPath.
    ...detectorFlagsForPath("paid"),
