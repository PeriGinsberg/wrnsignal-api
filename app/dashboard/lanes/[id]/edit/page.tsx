...
// Lane edit screen.
//
// Titles are the only writable field here — see app/api/lanes/[id]/route.ts for
// why. The rest of the config is shown because you cannot judge a title without
// it: the same phrase searched with a keyword and without one returns different
...
type Found = { title: string; count: number; already: boolean }
...
function locationLabel(l: Lane["location"]): string {
...
export default function LaneEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ;(async () => {
      const res = await authFetch(`/api/lanes/${encodeURIComponent(id)}`)
  ...
  const search = useCallback(async () => {
    ...
    const res = await authFetch(
      `/api/lanes/discover-titles?lane_id=${encodeURIComponent(id)}&phrase=${encodeURIComponent(p)}`
    )
    const j = await res.json().catch(() => ({}))
    setSearching(false)
    if (!res.ok || !j.ok) {
  ...
  if (!lane) {
    return (
      <div>
        <p style={{ color: T.ERROR, fontSize: 13 }}>{error || "Lane not found"}</p>
        <Link href="/dashboard/lanes" style={{ color: T.WRN_BLUE, fontSize: 13 }}>
          Back to Lane Review
        </Link>
      </div>
    )
  ...
  return (
    <div>
      ...
      {/* Read-only context. These shape every search on this screen. */}
      <div style={{ ...card, padding: "16px 18px", marginBottom: 18, display: "flex", gap: 28, flexWrap: "wrap" }}>
        <Fact label="Keyword" value={lane.keyword ?? "none"} dim={!lane.keyword} />
        <Fact label="Location" value={locationLabel(lane.location)} />
        <Fact label="Years max" value={lane.years_max == null ? "no ceiling" : String(lane.years_max)} />
        <Fact
          label="Excluded title words"
          value={lane.exclusions?.title_keywords?.join(", ") || "none"}
          dim={!lane.exclusions?.title_keywords?.length}
        />
        <Fact label="Status" value={lane.active ? "active" : "paused"} dim={!lane.active} />
      </div>

      <section style={{ ...card, padding: "18px 20px", marginBottom: 18 }}>
