...
**Auth:** Public.
**Purpose:** Create a Stripe one-time checkout session. Promotion codes are enabled (`allow_promotion_codes: true`) so customers can redeem any Promotion Code defined in the Stripe dashboard. When called from the mobile app with `source: "mobile"`, the session is tagged with `metadata.source = "mobile"` and its `success_url` routes to `/checkout/mobile-success` (which deep-links back into the app via `signalmobile://post-purchase`) instead of the web `/checkout/success` page. All attribution and match-quality fields from the request body are sanitized (≤500 chars, trimmed) and forwarded to both `session.metadata` and `payment_intent_data.metadata` for the Stripe webhook's downstream Conversion API fan-out. The endpoint also captures `client_ip` (from `x-forwarded-for` / `x-real-ip` / `cf-connecting-ip`) and `client_user_agent` (from the `user-agent` header) server-side and includes them in the same metadata — these are required for Meta / TikTok CAPI match quality and cannot be captured later at webhook time (the webhook request originates from Stripe's server, not the customer's browser). Empty values are dropped before being sent to Stripe so the metadata budget and dashboard stay clean.
**Request:** `{ email: string, source?: "mobile", utm_source?: string, utm_medium?: string, utm_campaign?: string, utm_content?: string, utm_term?: string, landing_page?: string, referrer?: string, fbclid?: string, ttclid?: string, gclid?: string, fbp?: string, fbc?: string, ttp?: string }` — all attribution fields optional.
...
**Purpose:** Handles two event types. (1) `checkout.session.completed` — creates or activates the `client_profiles` row; sets `stripe_customer_id`, `purchase_date`, `stripe_payment_intent_id`, `stripe_charge_id`; clears `refunded_at`; sends an auth email (OTP for mobile via `session.metadata.source === "mobile"`, otherwise a magic link to the web dashboard); inserts an idempotent row into `purchases` (keyed on `stripe_payment_intent_id`) with all 15 attribution fields from `session.metadata` — UTMs, landing_page, referrer, click IDs (fbclid/ttclid/gclid), pixel cookies (fbp/fbc/ttp), client_ip, client_user_agent — plus `amount_cents` and `currency` from the session; and schedules Meta / TikTok / Google Ads / GA4 Conversion API fan-out via `after()` so it runs post-response. Zero-amount purchases skip the CAPI fan-out (zero-value events would pollute ad-platform optimization) but still write the purchases row for record-keeping. (2) `charge.refunded` — looks up the profile by `stripe_customer_id` (fallback `stripe_payment_intent_id`), sets `active = false` and `refunded_at = now()` on the profile, updates the matching `purchases` row's `refunded_at`, and schedules refund events (Meta custom `Refund`, TikTok `Refund`, Google Ads `uploadConversionAdjustments` with `RETRACT`, GA4 `refund`) via `after()`. Refunds that cannot be matched to a `purchases` row (purchase predates feature) are logged with `charge_id`, `payment_intent_id`, and `email` for manual ad-platform reconciliation. Every Conversion API attempt writes a row to `conversion_log` regardless of success / skipped / error. Returns 200 as soon as signature verification and the synchronous Supabase writes complete.
